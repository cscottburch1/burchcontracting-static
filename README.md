...
`check-wrangler-config`, and `check-routing` against a `wrangler dev` it starts
and stops itself. It is the same command CI runs. Use `npm test -- --no-routing`
to skip the Worker.
...
trigger is commented out until the verify-and-rollback path has been exercised
once. The workflow builds, runs every gate, deploys, verifies against
production, and rolls back automatically if any check fails.
