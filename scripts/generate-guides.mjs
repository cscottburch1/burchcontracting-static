...
import { PERMIT_OFFICES, SC_BUILDING_CODES_COUNCIL_URL } from '../src/data/geo-aeo.js'
import {
...
for (const kind of Object.keys(KINDS)) {
  ...
  for (const guide of KINDS[kind].entries) {
    writeFileSync(resolve(dir, `${guide.slug}.html`), guidePage(guide, kind, relatedFor(guide, kind)), 'utf-8')
    written++
  }

  writeFileSync(resolve(dir, 'index.html'), hubPage(kind), 'utf-8')
  written++
}

console.log(`✅ Generated ${written} pages: ${COST_GUIDES.length} cost guides, ${ARTICLES.length} articles, 2 hubs.`)
