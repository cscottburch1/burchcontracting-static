...
import { SCOTT_PERSON_SCHEMA, ORGANIZATION_SCHEMA, LOCAL_BUSINESS_SCHEMA, WEBSITE_SCHEMA, articleSchema, webPageSchema } from '../src/data/site-schema.js'
...
function replaceOrInsertAfter(html, startMarker, endMarker, block, anchorRegex, anchorLabel) {
  ...
  if (html.includes(startMarker)) {
    ...
    // "$$", "$&", "$`", "$'" and "$<n>" in a replacement *string* as special
    // patterns, and `block` is generated content that can contain literal
    // dollar signs anywhere on this pricing-heavy site (this is exactly how
    // LOCAL_BUSINESS_SCHEMA's priceRange: '$$' silently became '$' the first
    // time JSON-LD content routed through this function). A function's
    // return value is always inserted literally — see the same fix applied
    // to every other html.replace(re, dynamicString) call in this file and
    // in generate-calculator-tables.mjs.
...
for (const relFile of [...FILES, ...SCHEMA_ONLY_FILES]) {
  ...
  // Same entity-decoding need as stripTags() above: this is read out of an
  // already-HTML-escaped attribute and serialized straight into JSON-LD.
  const description = decodeEntities(descMatch[1])
  const canonical = canonicalMatch[1]
  // og:image is already a plain absolute URL (not HTML-entity-bearing text),
  // so no decoding needed here the way headline/description require it.
