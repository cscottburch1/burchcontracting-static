...
const EXTRA_TABLE_BUILDERS = {
  ...
  'services.html': () => costOverviewTableHtml('Typical Cost by Service'),
...
${extraFaqs
  ...
  if (tableBuilder && !html.includes(TABLE_START)) {
    const tableBlock = tableBuilder()
    if (!html.includes('    </main>')) throw new Error(`${relFile}: no </main> found for table insertion`)
